...
export default {
  name: "TablaLogin",
  data() {
    //Esto resetea el formulario al cargar la pagina
    return {
      dni: "", //Asigna al atributo dni el valor del input(supongo)
      pass: "", //Asigna al atributo pass el valor del input(supongo)
    };
  },
    ...
    async iniciarSesion() {
      try {
        ...
        if (this.dni === "" || this.pass === "") {
          await Swal.fire({
            icon: "error",
            title: "Los campos no deben ir vacíos",
