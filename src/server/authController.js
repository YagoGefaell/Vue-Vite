...
export const login = async (req, res) => {
  ...
  try {
    ...
    if (historico)
      return res.status(400).json({ message: "Usuario Historico" });
...
export const checkAdmin = (req, res) => {
  ...
  if (!authHeader) {
    return res
      .status(401)
      .json({ isAdmin: false, mensaje: "Token no proporcionado" });
  }
  ...
  try {
    ...
    return res
      .status(403)
      .json({ isAdmin: false, mensaje: "Token inválido o expirado" });
  }
};
